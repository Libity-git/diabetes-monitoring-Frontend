// 📁 src/pages/liff/LiffCaregiverReport.jsx
...
import { Activity, Loader2, CheckCircle, AlertTriangle, Droplets, Heart, Users, UserPlus, ArrowLeft } from 'lucide-react';
import axios from 'axios';

const API_URL = process.env.REACT_APP_API_URL || 'https://diabetes-monitoring-backend.onrender.com';

const LiffCaregiverReport = () => {
    ...
    const init = async () => {
      ...
      if (initialized) {
        ...
        if (userProfile) {
          ...
          // Check if registered as caregiver
          try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Submit report via caregiver API
      await axios.post(`${API_URL}/api/caregiver/report`, {
      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center p-4">
        <Card className="w-full max-w-md border-0 shadow-xl">
          <CardContent className="pt-8 pb-8 text-center">
            ...
            <p className="text-slate-600 mb-4">กรุณาลงทะเบียนเป็นผู้ดูแลและเพิ่มผู้ป่วยก่อน</p>
            <Button 
              onClick={closeWindow}
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center p-4">
        <Card className="w-full max-w-md border-0 shadow-xl">
          <CardContent className="pt-8 pb-8 text-center">
            ...
            <p className="text-sm text-slate-500">บันทึกข้อมูลให้ {selectedPatient?.name} เรียบร้อยแล้ว</p>
      ...
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-100 flex items-center justify-center p-4">
        <Card className="w-full max-w-md border-0 shadow-xl">
          <CardHeader className="text-center pb-2">
            ...
            <CardTitle className="text-xl">เลือกผู้ป่วยที่ต้องการส่งข้อมูล</CardTitle>
          ...
          <CardContent className="space-y-3">
            {caregiver?.patients?.map((patient) => (
              <button
                key={patient.id}
                className="w-full p-4 text-left bg-slate-50 hover:bg-indigo-50 rounded-xl border border-slate-200 hover:border-indigo-300 transition-colors"
                onClick={() => setSelectedPatient(patient)}
              >
                <p className="font-semibold text-slate-800">{patient.name}</p>
                <p className="text-sm text-slate-500">อายุ {patient.age} ปี • เพศ{patient.gender}</p>
              </button>
            ))}
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-100 flex items-center justify-center p-4">
      <Card className="w-full max-w-md border-0 shadow-xl">
        <CardHeader className="text-center pb-2">
          ...
          <CardTitle className="text-xl">ส่งผลตรวจแทน</CardTitle>
          <p className="text-sm text-slate-500">กำลังส่งให้ {selectedPatient?.name}</p>
        </CardHeader>
        <CardContent>
...
export default LiffCaregiverReport;
